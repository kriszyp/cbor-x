...
Records are also supported in conjunction with keyMaps, but these are disabled by default when keyMaps are specified as the the use of the two features together has not yet been optimised and is currently less efficient than using just one or the other.
